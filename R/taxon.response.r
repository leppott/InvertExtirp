...
"taxon.response" <- function(spdata, envdata,  sp.siteid="Sample.ID", species="GENUS", 
        ...
        xlabs=expression(paste("Conductivity ( ", mu, "S/cm)")), log.x=TRUE, rounder=0, taus=c(0,95,100), nbin = 61) {
  graphics.off()
...
# Build site species matrix for only taxa with enough observations 
    ...
    for (j in 1:ntaxa) {                                      # all taxa in j list
...
#       segments(full[1], full[2],full[1],0, col="blue", lty= "dashed")   # full range      
       if(lim =="GAM") { abline(v= lrm3.95f[1], lty= "dashed", col="red")
       } else if(lim =="CDF"){
